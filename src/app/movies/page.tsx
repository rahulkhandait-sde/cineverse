...
function MoviesPageContent() {
	...
	useEffect(() => {
		...
		if (searchQuery.trim() || genre !== "all") {
			...
			const genrePart =
				genre !== "all"
					? `(${movieGenres.find((g) => g.value === genre)?.label} Genre)`
					: "";
			document.title =
					...
					<AnimatePresence mode='wait'>
						{isLoadingContent ? (
							...
							<>
									...
									<div className='flex items-center gap-6'>
										<div className='flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300'>
											...
											<span className='font-semibold'>
												{moviesToDisplay.length} movies of {totalResults}{" "}
												results
											</span>
										</div>
										{actorData &&
								...
								{hasNextPage &&
									genre === "all" && ( // Ensure only loads more if no genre filter applied
										<motion.div
											initial={{ opacity: 0, y: 20 }}
											animate={{ opacity: 1, y: 0 }}
											transition={{ duration: 0.4 }}
											...
											<div className='w-full max-w-lg'>
												...
												<div className='text-center mt-2 text-xs text-gray-500 font-medium'>
													{Math.round(
														(moviesToDisplay.length / parseInt(totalResults)) *
															100
													)}
													% Complete
												</div>
										...
										<div className='relative'>
											...
											<div className='relative z-10'>
												...
												<p className='text-lg font-semibold mb-2 text-red-600 dark:text-red-400'>
													You&apos;ve explored all {moviesToDisplay.length}{" "}
													premium results!
												</p>
