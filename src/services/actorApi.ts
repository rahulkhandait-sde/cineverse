...
export const actorApi = {
	...
	searchActors: async (query: string): Promise<Actor[]> => {
		...
		const results = actorDataset.filter(
			(actor) =>
	...
	getMoviesByActor: async (actorName: string): Promise<SearchResponse> => {
		...
		console.log(
			`🔍 Looking for actor: "${actorName}" -> cleaned: "${cleanActorName}"`
		);

		const actor = actorDataset.find(
			(a) =>
		...
		try {
			...
			const mockMovies = actor.movies.map((movieTitle, index) => ({
				...
				Poster: `https://via.placeholder.com/300x450/1f2937/ffffff?text=${encodeURIComponent(
					movieTitle
				)}`,
				Type: "movie",
