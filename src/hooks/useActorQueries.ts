...
import { SearchResponse, Actor } from "../types/movie";
...
export const useInfiniteMoviesByActor = (actorName: string) => {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 1 }) => {
            ...
            return await actorApi.getMoviesByActorWithPagination(actorName, pageParam);
        },
...
export const useCombinedSearch = (query: string, searchType: 'movie' | 'actor' | 'both' = 'both') => {
    const movieQuery = useQuery({
        ...
        enabled: Boolean(query.trim()) && (searchType === 'movie' || searchType === 'both'),
        staleTime: 5 * 60 * 1000,
    });
    ...
    const actorQuery = useQuery({
        ...
        enabled: Boolean(query.trim()) && (searchType === 'actor' || searchType === 'both'),
        staleTime: 5 * 60 * 1000,
    });
